...
void ShowUsage(int outtype)
{
    if(outtype == 0)
    {
        printf("Print the forensic imaging log for a wombat forensic and wombat logical image.\n\n");
        printf("Usage :\n");
        printf("\twombatlog IMAGE_FILE\n\n");
        printf("IMAGE_FILE\t: the file name for the wombat forensic or wombat logical image.\n");
        printf("Arguments :\n");
        printf("-v\t: Prints version information\n");
        printf("-h\t: Prints help information\n\n");
        printf("Example Usage :\n");
        printf("wombatlog item1.wfi\n");
    }
    else if(outtype == 1)
    {
...
int main(int argc, char* argv[])
{
	...
	else
	{
	    ...
	    std::string wltgrawimg = wltgimg.substr(0, found) + ".log";

...
int main(int argc, char* argv[])
{
    ...
    // THIS WORKS, JUST NEED TO BE BUILD IN ERROR CONTROLS AND HELPER MENU
    std::string wfipath = std::string(argv[1]);
    std::string wfidd = wfipath + "/" + wfipath + ".dd";
    std::string wfilog = wfipath + "/" + wfipath + ".log";
    std::string wfistr = wfipath + ".wfi";
    int zerr = 0;
    zip_t* wfizip = zip_open(wfistr.c_str(), ZIP_CREATE, &zerr);
    zip_int64_t zadderr = zip_dir_add(wfizip, wfipath.c_str(), ZIP_FL_ENC_UTF_8);
    zip_source_t* logsource = zip_source_file(wfizip, wfilog.c_str(), 0, ZIP_LENGTH_TO_END);
    zip_file_add(wfizip, wfilog.c_str(), logsource, ZIP_FL_OVERWRITE|ZIP_FL_ENC_UTF_8);
    zip_source_t* ddsource = zip_source_file(wfizip, wfidd.c_str(), 0, ZIP_LENGTH_TO_END);
    zip_file_add(wfizip, wfidd.c_str(), ddsource, ZIP_FL_OVERWRITE|ZIP_FL_ENC_UTF_8);
    //zadderr = zip_file_add(wfizip,  
